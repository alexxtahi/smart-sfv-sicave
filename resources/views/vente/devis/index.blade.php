...
<div class="col-md-3">
    <div class="form-group">
       <input type="text" class="form-control" id="searchByFacture" placeholder="Rechercher par N° devis">
    </div>
</div>
<div class="col-md-3">
    <div class="form-group">
...
<table id="table" class="table table-warning table-striped box box-primary"
               ...
               data-search="false"
               data-toggle="table"
               data-url="{{url('vente',['action'=>'liste-devis'])}}"
               data-unique-id="id"
    ...
    <thead>
        <tr>
            ...
            <th data-field="date_deviss">Date </th>
            <th data-field="numero_devis">Facture </th>
            ...
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    ...
                    <span style="font-size: 16px;">
                        ...
                        Gestion des devis et factures proformas
                    </span>
                </div>
                <div class="modal-body ">
                    <input type="text" class="hidden" id="idDevis" name="idDevis" ng-hide="true" ng-model="devis.id"/>
                    @csrf
                    <div class="row">
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" ng-model="devis.date_deviss" id="date_devis" name="date_devis" placeholder="Ex: 01-01-1994" value="<?= date('d-m-Y'); ?>" required>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="depot_id" id="depot_id" ng-model="devis.depot_id" ng-init="devis.depot_id=''" class="form-control" required>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <label>
                                <input type="radio" checked="checked" name="proformat_devis" value="devis" ng-model="devis.proformat_devis">&nbsp;&nbsp;Devis
                            </label>
                        </div>
                        <div class="col-md-4">
                            <label>
                                <input type="radio" name="proformat_devis" value="facture_proforma" ng-model="devis.proformat_devis">&nbsp;&nbsp;Facture proforma
                            </label>
                        </div>
                        <div class="col-md-2"></div>
                    </div>
        ...
        <form id="formAjoutArticle" ng-controller="formAjoutArticleCtrl" action="#">
            <div class="modal-content">
                ...
                <div class="modal-body ">
                   ...
                   <input type="text" class="hidden" id="devis"  name="devis_id"/>
                   <div class="row">
                       <div class="col-md-3">
                           <div class="form-group">
                               <label>Code Barre</label>
    ...
    <div class="modal-dialog ">
        <form id="formSupprimer" ng-controller="formSupprimerCtrl" action="#">
            <div class="modal-content">
                ...
                @csrf
                <div class="modal-body ">
                    <input type="text" class="hidden" id="idDevisSupprimer"  ng-model="devis.id"/>
                    <div class="clearfix">
                        <div class="text-center question"><i class="fa fa-question-circle fa-2x"></i> Etes vous certains de vouloir supprimer @{{devis.numero_devis =="devis" ? ' le devis' : ' la facture proforma'}} <br/><b> @{{devis.numero_devis}}</b></div>
                        <div class="text-center vertical processing">Suppression en cours</div>
                        <div class="pull-right">
                            <button type="button" data-dismiss="modal" class="btn btn-default btn-sm">Non</button>
                            <button type="submit" class="btn btn-danger btn-sm ">Oui</button>
    ...
    <div class="modal-dialog" style="width:65%">
            <div class="modal-content">
                <div class="modal-header bg-yellow">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    Panier @{{devis.numero_devis =="devis" ? ' du devis' : ' de la facture proforma'}} <b>@{{devis.numero_devis}}</b>
                </div>
                @csrf
                <div class="modal-body ">
...
<script type="text/javascript">
    ...
    appSmarty.controller('formAjoutCtrl', function ($scope) {
        $scope.populateForm = function (devis) {
            $scope.devis = devis;
        };
    ...
    appSmarty.controller('panierArticleCtrl', function ($scope) {
        $scope.populateFormPanier = function (devis) {
            $scope.devis = devis;
        };
    });
    ...
    $(function () {
        ...
        $('#searchByDate, #date_devis').datetimepicker({
            timepicker: false,
            formatDate: 'd-m-Y',
            format: 'd-m-Y',
            local : 'fr',
        ...
        $("#searchByClient").change(function (e) {
            ...
            if(client == 0){
                 $table.bootstrapTable('refreshOptions', {url: "{{url('vente', ['action' => 'liste-devis'])}}"});
            }else{
               $table.bootstrapTable('refreshOptions', {url: '../vente/liste-devis-by-client/' + client});
            }
        });
        ...
        $("#searchByDate").change(function (e) {
            ...
            if(date == ""){
                ...
                $table.bootstrapTable('refreshOptions', {url: '../vente/liste-devis-by-date/' + date});
            }
        });
        ...
        $("#searchByFacture").keyup(function (e) {
            ...
            if(numero_facture == ""){
               ...
               $table.bootstrapTable('refreshOptions', {url: '../vente/liste-devis/' + numero_facture});
            }
        });

        $("#btnModalAjoutArticle").on("click", function () {
            ajoutArticle = true;
            var devis = $("#idDevis").val();
        ...
        $("#formAjout").submit(function (e) {
            ...
            if (ajout==true) {
                var methode = 'POST';
                var url = "{{route('vente.devis.store')}}";
                var formData = new FormData($(this)[0]);
                createFormData(formData, 'monPanier', monPanier);
            }else{
               var methode = 'POST';
                var url = "{{route('vente.update-devis')}}";
                 var formData = new FormData($(this)[0]);
            }
            editerDevisAction(methode, url, $(this), formData, $ajaxLoader, $table, ajout);
        });
        $("#formAjoutArticle").submit(function (e) {
            ...
            if (ajoutArticle==true) {
                var methode = 'POST';
                var url = "{{route('vente.articles-devis.store')}}";
             }else{
                var id = $("#idArticleModifier").val();
                var methode = 'PUT';
                var url = 'articles-devis/' + id;
             }
            editerDevisArticlesAction(methode, url, $(this), $(this).serialize(), $ajaxLoader, $tableArticle,$table, ajoutArticle);
        });

        $("#formSupprimer").submit(function (e) {
            e.preventDefault();
            var id = $("#idDevisSupprimer").val();
            var formData = $(this).serialize();
            var $question = $("#formSupprimer .question");
            var $ajaxLoader = $("#formSupprimer .processing");
            supprimerAction('devis/' + id, $(this).serialize(), $question, $ajaxLoader,$table);
        });
        $("#formSupprimerArticle").submit(function (e) {
            ...
            supprimerArticleAction('articles-devis/' + id, $(this).serialize(), $question, $ajaxLoader, $tableArticle, $table);
        });
    });
    ...
    function updateRow(idDevis) {
        ajout = false;
        var $scope = angular.element($("#formAjout")).scope();
        var devis =_.findWhere(rows, {id: idDevis});
         $scope.$apply(function () {
            $scope.populateForm(devis);
        });

        $("#client_id").select2("val", devis.client_id);
        $("#idDevis").val(devis.id);
        $("#depot_id").prop('disabled',true);
        $tableArticle.bootstrapTable('refreshOptions', {url: "../vente/liste-articles-devis/" + idDevis});
        $("#div_enregistrement").hide();
        $("#div_update").show();
        $(".bs-modal-ajout").modal("show");
    }
    function deleteRow(idDevis){
    ...
    function panierRow(idDevis){
        var $scope = angular.element($("#panierArticle")).scope();
        var devis =_.findWhere(rows, {id: idDevis});
         $scope.$apply(function () {
            $scope.populateFormPanier(devis);
        });
        $tablePanierArticle.bootstrapTable('refreshOptions', {url: "../vente/liste-articles-devis/" + idDevis});

        $(".bs-modal-panier").modal("show");
    }

    function facturePrintRow(idDevis){
        window.open("devis-pdf/" + idDevis ,'_blank')
    }
    ...
    function editerDevisAction(methode, url, $formObject, formData, $ajoutLoader, $table, ajout = true) {
        jQuery.ajax({
            type: methode,
            url: url,
            cache: false,
            ...
            success:function (reponse, textStatus, xhr){
                if (reponse.code === 1) {
                    ...
                    $("#row_regle").hide();
                    window.open("devis-pdf/" + reponse.data.id ,'_blank')
                    location.reload();
    ...
    function editerDevisArticlesAction(methode, url, $formObject, formData, $ajoutLoader, $table,$tableVente, ajoutArticle = true) {
        jQuery.ajax({
            type: methode,
            url: url,
            cache: false,
