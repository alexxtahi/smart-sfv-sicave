...
<div class="col-md-4">
    <select class="form-control" id="searchByCarte">
        <option value="0">-- Toutes les cartes --</option>
        @foreach($cartes as $carte)
        <option value="{{$carte->id}}"> {{$carte->libelle_carte_fidelite.' '.($carte->reduction*100).'%'}}</option>
        @endforeach
    </select>
</div>
...
<table id="table" class="table table-primary table-striped box box-primary"
               ...
               data-search="true"
               data-toggle="table"
               data-url="{{url('crm',['action'=>'liste-compte-carte'])}}"
               data-unique-id="id"
    ...
    <thead>
        <tr>
            ...
            <th data-field="numero_compte" data-searchable="true">N° de carte </th>
            <th data-formatter="carteFormatter">Carte </th>
                    ...
                    <div class="col-md-5">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="carte_id" id="carte_id" ng-model="compte.carte_id" class="form-control" required>
                                        <option value="" ng-show="false">-- Selectionner le type de carte --</option>
                                        @foreach($cartes as $carte)
                                            <option value="{{$carte->id}}"> {{$carte->libelle_carte_fidelite.' '.($carte->reduction*100).'%'}}</option>
                                        @endforeach
                                    </select>
        ...
        <form id="formRecharge" ng-controller="formRechargeCtrl" action="#">
            <div class="modal-content">
                ...
                <div class="modal-body ">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="text-left">Carte N° : <b>@{{compte.numero_compte}}</b></div>
                        </div>
                        <div class="col-md-8">
                            <div class="text-left">Client : <b>@{{compte.client_id ? compte.client.code_client + ' - ' + compte.client.full_name_client : " Anonyme"}}</b></div>
                        </div>
                    </div>
                    <hr/>
                    <div class="row">
...
<script type="text/javascript">
       ...
       $table.on('load-success.bs.table', function (e, data) {
        ...
        $("#searchByClient").change(function (e) {
            ...
            if(client == 0){
              ...
              $table.bootstrapTable('refreshOptions', {url: '../crm/liste-compte-carte-by-client/' + client});
            }
        });

        $("#searchByCarte").change(function (e) {
            var carte = $("#searchByCarte").val();

            if(carte == 0){
                $table.bootstrapTable('refreshOptions', {url: "{{url('crm', ['action' => 'liste-compte-carte'])}}"});
            }else{
              $table.bootstrapTable('refreshOptions', {url: '../crm/liste-compte-carte/' + carte});
            }
        });
        ...
        $("#formSupprimer").submit(function (e) {
            ...
            supprimerAction('compte-clients/' + id, $(this).serialize(), $question, $ajaxLoader, $table);
        });
    });
    ...
    function imprimePdf(){
        ...
        if(carte != 0 && client == 0){
            window.open("liste-compte-carte-by-carte-pdf/" + carte, '_blank');
        }
        if(carte == 0 && client != 0){
            window.open("liste-compte-carte-by-client-pdf/" + client, '_blank');
        }
        if(carte != 0 && client != 0){
            window.open("liste-compte-carte-by-carte-pdf/" + carte, '_blank');
    ...
    function printRow(idCompte){
        window.open("carte-fidelite-pdf/" + idCompte,'_blank');
    }
    function carteFormatter(id, row){
        return row.carte.libelle_carte_fidelite + ' - ' + (row.carte.reduction*100) + '%';
    }
    ...
    function optionFormatter(id, row) {
        return '<button class="btn btn-xs btn-primary" data-placement="left" data-toggle="tooltip" title="Modifier" onClick="javascript:updateRow(' + id + ');"><i class="fa fa-edit"></i></button>\n\
                ...
                <button class="btn btn-xs btn-warning" data-placement="left" data-toggle="tooltip" title="Imprimer la carte" onClick="javascript:printRow(' + id + ');"><i class="fa fa-print"></i></button>';
    }
