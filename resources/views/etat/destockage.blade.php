...
<script type="text/javascript">
    ...
    $(function () {
       $table.on('load-success.bs.table', function (e, data) {
        ...
        $("#searchByDepot").change(function (e) {
            ...
            if(depot == 0 && dateDebut!="" && dateFin != ""){
                $table.bootstrapTable('refreshOptions', {url: '../stock/liste-destockages-by-periode/' + dateDebut + '/' + dateFin});
            }
            if(depot != 0 && dateDebut!="" && dateFin != ""){
              $table.bootstrapTable('refreshOptions', {url: '../stock/liste-destockages-by-periode-depot/' + dateDebut + '/' + dateFin + '/' + depot});
            }
        });
    ...
    function imprimePdf(){
        ...
        if(dateDebut=='' && dateFin=='' && depot!=0){
            window.open("liste-destockages-by-depot-pdf/" + depot,'_blank');
        }
        if(dateDebut!='' && dateFin!='' && depot==0){
            window.open("liste-destockages-by-periode-pdf/" + dateDebut + '/' + dateFin ,'_blank');
        }
        if(depot != 0 && dateDebut!="" && dateFin != ""){
            window.open("liste-destockages-by-periode-depot-pdf/" + dateDebut + '/' + dateFin + '/' + depot ,'_blank');
        }
    }
