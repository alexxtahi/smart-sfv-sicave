...
<table id="table" class="table table-primary table-striped box box-primary"
               ...
               data-search="false"
               data-toggle="table"
               data-url="{{url('comptabilite',['action'=>'liste-declarations-tva'])}}"
...
<script type="text/javascript">
       ...
       $table.on('load-success.bs.table', function (e, data) {
        ...
        $("#dateDebut, #dateFin").change(function (e) {
            ...
            if(dateDebut=='' && dateFin=='' && depot==0){
                $table.bootstrapTable('refreshOptions', {url: "{{url('comptabilite', ['action' => 'liste-declarations-tva'])}}"});
            }
            if(dateDebut!='' && dateFin!='' && depot==0){
                $table.bootstrapTable('refreshOptions', {url: '../comptabilite/liste-declarations-tva-by-periode/' + dateDebut + '/' + dateFin});
            }
            if(dateDebut=='' && dateFin=='' && depot!=0){
                $table.bootstrapTable('refreshOptions', {url: '../comptabilite/liste-declarations-tva-by-depot/' + depot});
            }
            if(dateDebut!='' && dateFin!='' && depot!=0){
                $table.bootstrapTable('refreshOptions', {url: '../comptabilite/liste-declarations-tva-by-depot-periode/'+ depot + '/' + dateDebut + '/' + dateFin});
            }
        });
        ...
        $("#formSupprimer").submit(function (e) {
            ...
            supprimerAction('declarations-tva/' + id, $(this).serialize(), $question, $ajaxLoader, $table);
        });
    ...
    function listeRow(idDeclaration) {
        ...
        $tableListe.bootstrapTable('refreshOptions', {url: "../comptabilite/liste-tickets-declares/" + idDeclaration});

        $(".bs-modal-liste").modal("show");
    }

    function printRow(idDeclaration){
        window.open("../comptabilite/tickets-declares-pdf/" + idDeclaration,'_blank');
    }
