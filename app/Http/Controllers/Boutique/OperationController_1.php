    ...
    public function listeOperationsByCaisse($caisse)
    {
        ...
        if (Auth::user()->role != 'Caissier') {
            $operations = Operation::with('vente', 'bon_commande')
                ...
                ->whereDate('operations.date_operation', $date_jour)
        ...
        if ($request->isMethod('post') && $request->input('montant_operation')) {
            ...
            try {
                ...
                if (isset($data['vente_id']) && !empty($data['vente_id'])) {
                    ...
                    if ($data['montant_operation'] > $montantDu) {
                        return response()->json(["code" => 0, "msg" => "Le montant saisi est supérieur au montant dû sur cette facture qui est " . number_format($montantDu, 0, ',', ' '), "data" => null]);
                    }
                }
                ...
                if (isset($data['bon_commande_id']) && !empty($data['bon_commande_id'])) {
                    ...
                    if ($data['montant_operation'] > $montantDu) {
                        return response()->json(["code" => 0, "msg" => "Le montant saisi est supérieur au montant dû qui est " . number_format($montantDu, 0, ',', ' '), "data" => null]);
                    }
                }
                ...
                $operation->concerne = $data['concerne'];
                $operation->caisse_ouverte_id = $caisse_ouverte_id;
                $operation->bon_commande_id = isset($data['bon_commande_id']) && !empty($data['bon_commande_id']) ? $data['bon_commande_id'] : null;
                $operation->vente_id = isset($data['vente_id']) && !empty($data['vente_id']) ? $data['vente_id'] : null;
    ...
    public function update(Request $request, Operation $operation)
    {
        ...
        if ($operation) {
            ...
            try {
                ...
                //*****Vérifions si le montant du est inferieur au montant saisi****//
                //Si l'operation concerne un client
                if (isset($data['vente_id']) && !empty($data['vente_id'])) {
                    $Vente = Vente::with('client', 'depot')
                        ->join('article_ventes', 'article_ventes.vente_id', '=', 'ventes.id')->Where('article_ventes.deleted_at', null)
                        ->select('ventes.*', DB::raw('sum(article_ventes.quantite*article_ventes.prix) as sommeTotale'), DB::raw('DATE_FORMAT(ventes.date_vente, "%d-%m-%Y") as date_ventes'))
